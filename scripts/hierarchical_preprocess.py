...
if __name__ == "__main__":
    ...
    for file, prev_file in tqdm(zip(file_paths, prev_path), total=len(file_paths), desc="Processing hourly trade files"):
        ...
        df_agg["lag1_abs_log_return"] = df_agg["log_return"].shift(1).apply(abs)
        df_agg["lag1_trade_count"] = df_agg["trade_count"].shift(1)
        df_agg["lag1_log_volume"] = df_agg["log_volume"].shift(1)
        df_agg["lag1_buy_sell_imbalance"] = df_agg["buy_sell_imbalance"].shift(1)
        ...
        df_agg = df_agg.drop(["close_price", "open_price", "log_volume", "buy_sell_imbalance", "log_return", "prev_close_price"], axis =1)
        if prev_file is not None:
...
print(f"\nTrades\t:\t{len(trades)}\n")
output_file = os.path.join(output_dir, f"hierarchical_{group}_data.csv")
